...
export const lineSchema=z.object({variantCode:z.string().max(200).optional(),image:z.string().max(2000).optional(),stockSnapshot:z.number().int().nonnegative().nullable().optional(),stockOverride:z.boolean().optional(),catalogueUpdatedAt:z.string().max(100).optional(),brandingMethod:z.string().max(200).optional(),brandingPosition:z.string().max(200).optional(),brandingCode:z.string().max(100).optional(),brandingPositions:z.number().int().min(1).max(100).optional(),productCode:z.string().max(200).default(""),description:z.string().trim().min(1).max(1500),colour:z.string().max(100).default(""),size:z.string().max(100).default(""),unitPriceCents:cents,quantity:z.number().int().min(1).max(1000000),discountBps:z.number().int().min(0).max(10000).default(0),brandingCents:cents.default(0),setupCents:cents.default(0),otherCents:cents.default(0),taxable:z.boolean().default(true)}).strict();
