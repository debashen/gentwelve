...
export default function AnalyticsDashboard(){
 ...
 useEffect(()=>{const controller=new AbortController();setLoading(true);setError("");fetch(`/api/admin/analytics?days=${days}`,{cache:"no-store",signal:controller.signal}).then(async response=>{const payload=await response.json() as Payload;if(!response.ok)throw new Error(payload.error||"Could not load enquiry activity.");setData(payload)}).catch(reason=>{if(!(reason instanceof DOMException&&reason.name==="AbortError"))setError(reason instanceof Error?reason.message:"Could not load enquiry activity.")}).finally(()=>setLoading(false));return()=>controller.abort()},[days]);
