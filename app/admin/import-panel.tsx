...
type Result={ok?:boolean;id?:number;runId?:number;dataset?:Dataset;received?:number;stored?:number;error?:string;status?:string};
...
export default function ImportPanel(){
 ...
 let current=overview.runs[dataset];
  let runId=current?.status==="running"?current.id:undefined;
 ...
 async function importProducts(){setBusy("products");setError("");setNotice("");try{await execute("products");await execute("variants");await execute("enrichment");await refresh();setNotice("Full catalogue reconciliation complete.")}catch(reason){await refresh().catch(()=>{});setError(reason instanceof Error?reason.message:"Product reconciliation paused.")}finally{setBusy(null);setActiveDataset(null)}}
 async function updateEverything(){setBusy("all");setError("");setNotice("");try{if(overview.runs.stock?.status==="running")await execute("stock");else if(overview.runs.prices?.status==="running"){await execute("prices");await execute("stock")}else{await execute("products","changes");await execute("prices");await execute("stock")}await refresh();setNotice("Catalogue update complete. Product changes, prices and stock are current.")}catch(reason){await refresh().catch(()=>{});setError(reason instanceof Error?reason.message:"Catalogue update paused.")}finally{setBusy(null);setActiveDataset(null)}}
 ...
 return <div className="admin-card admin-dashboard">
  ...
  {productComplete&&<section className="daily-update"><div><span>DAILY MAINTENANCE</span><h2>Update everything in one go</h2><p>Apply Amrod product changes, refresh selling prices and replace current stock figures.</p></div><button onClick={updateEverything} disabled={Boolean(busy)}>{busy==="all"?<LoaderCircle className="spin"/>:<RefreshCw/>}{busy==="all"?"Updating…":"Update catalogue now"}</button></section>}
