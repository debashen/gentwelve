...
export async function GET(request:Request){
 ...
 const [summary,daily,topProducts,searches,filters]=await Promise.all([
  ...
  env.DB.prepare("SELECT lower(search_query) AS term,COUNT(*) AS count FROM catalogue_events WHERE created_at>=? AND event_type='search' AND search_query IS NOT NULL AND search_query!='' GROUP BY lower(search_query) ORDER BY count DESC LIMIT 10").bind(since).all<Record<string,unknown>>(),
