...
export async function GET(request:Request) {
  ...
  if(search){conditions.push("(p.name ILIKE ? OR p.supplier_code ILIKE ? OR p.brand ILIKE ?)");const value=`%${search}%`;bindings.push(value,value,value)}
  ...
  const [result,categories,total,eligible]=await Promise.all([
    env.DB.prepare(`SELECT p.id,p.supplier_code AS code,p.name,p.category,p.brand,p.image_url AS image,p.public_price_cents AS priceCents,p.minimum_quantity AS minimumQuantity,p.curated,p.featured,p.trending,p.new_arrival AS newArrival,p.display_priority AS displayPriority,COALESCE((SELECT SUM(v.stock_quantity) FROM variants v WHERE v.product_code=p.supplier_code AND v.active=1 AND v.stock_quantity IS NOT NULL),0) AS stock,CASE WHEN p.public_price_cents IS NOT NULL AND p.image_url IS NOT NULL AND p.image_url!='' AND p.category IS NOT NULL AND p.category!='' AND EXISTS (SELECT 1 FROM variants v WHERE v.product_code=p.supplier_code AND v.active=1 AND COALESCE(v.stock_quantity,0)>0) THEN 1 ELSE 0 END AS eligible FROM products p WHERE ${conditions.join(" AND ")} ORDER BY p.curated DESC,p.featured DESC,p.trending DESC,p.new_arrival DESC,p.display_priority DESC,p.name ASC LIMIT ? OFFSET ?`).bind(...bindings,pageSize,page*pageSize).all<Record<string,unknown>>(),
