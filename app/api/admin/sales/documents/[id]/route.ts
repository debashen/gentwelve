...
export async function GET(request:Request,context:Context){try{
 await admin(request);const id=z.string().uuid().parse((await context.params).id);const doc=await getDocument(id);
 if(new URL(request.url).searchParams.get("format")==="pdf"){const pdf=await renderDocumentPdf(doc);return new NextResponse(new Uint8Array(pdf),{headers:{...noStore,"Content-Type":"application/pdf","Content-Disposition":`attachment; filename="${doc.number||"DRAFT"}.pdf"`}})}
 ...
 return NextResponse.json({document:doc,status:effectiveStatus(doc),related:related.results,events:events.results},{headers:noStore});
