...
export async function GET(request:Request,context:Context){try{const doc=await getPublicDocument((await context.params).token);const pdf=await renderDocumentPdf(doc);return new NextResponse(new Uint8Array(pdf),{headers:{...noStore,"Content-Type":"application/pdf","Content-Disposition":`attachment; filename="${doc.number}.pdf"`}})}catch(e){return fail(e)}}
