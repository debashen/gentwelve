...
import { documentPaid } from "@/lib/sales/payments";
export const dynamic="force-dynamic";
type Context={params:Promise<{token:string}>};
export async function GET(request:Request,context:Context){try{const doc=await getPublicDocument((await context.params).token);const pdf=await renderDocumentPdf(doc,await documentPaid(doc));return new NextResponse(new Uint8Array(pdf),{headers:{...noStore,"Content-Type":"application/pdf","Content-Disposition":`attachment; filename="${doc.number}.pdf"`}})}catch(e){return fail(e)}}
